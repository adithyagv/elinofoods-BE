...
router.post("/addingredient", async (req, res) => {
    ...
    try {
        ...
        return res.json({ success: true, message: "Ingredient added successfully", ingredient: result });
    }
    catch (error) {
...
router.put("/updateingredient/:id", async (req, res) => {
    ...
    try {
        const collection = db.collection("ingredients");
        const result = await collection.updateOne({ ingredient_id: id }, { $set: updateData });
